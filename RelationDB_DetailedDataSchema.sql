...
CHECK (eMail ~ '^[A-Za-z0-9][A-Za-z0-9._-]*@[A-Za-z][A-Za-z0-9-]*\.[A-Za-z]{2,}$');
...
CREATE TABLE catalogue( -- опис каталога
    ...
    dataCreated DATE, -- дата створення каталога
    user_id SMALLINT -- N користувача
);
...
ALTER TABLE catalogue
ALTER COLUMN dataCreated SET NOT NULL;
...
CREATE TABLE document( -- опис документа
    ...
    dataCreated DATE, -- дата створення документа
...
CREATE TABLE keyword( -- опис ключового слова
...
-- обмеження зовнішнього ключа
ALTER TABLE keyword 
ADD CONSTRAINT keyword_users_fk
FOREIGN KEY (user_id)
REFERENCES users (user_id);

ALTER TABLE keyword 
ADD CONSTRAINT keyword_document_fk
FOREIGN KEY (document_id)
REFERENCES document (document_id);

-- обмеження потенційного ключа
ALTER TABLE keyword
ADD CONSTRAINT keyword_document_word_unique 
UNIQUE (document_id,word);

CREATE TABLE important_text( -- опис виділеного тексту
    text_id SMALLINT, -- N виділеного тексту
    textPart TEXT, -- виділений текст 
    user_id SMALLINT, -- N користувача
    document_id SMALLINT -- N документа
);
...
-- обмеження вмісту стовпчиків таблиці
ALTER TABLE important_text
ALTER COLUMN textPart SET NOT NULL;

ALTER TABLE important_text
ADD CONSTRAINT important_text_text_part_constraint
CHECK (octet_length(textPart) < 5 * 1024 * 1024);
...
-- обмеження потенційного ключа
ALTER TABLE important_text
ADD CONSTRAINT important_text_document_textpart_unique 
UNIQUE (document_id,textPart);

CREATE TABLE note( -- опис примітки
    note_id SMALLINT, -- N примітки
    title VARCHAR(149), -- назва примітки
    textPart TEXT, -- зміст примітки
    dataCreated DATE, -- дата створення примітки
    user_id SMALLINT -- N користувача
);
...
CREATE TABLE doctor( -- опис лікаря
    ...
    workExperience SMALLINT, -- стаж роботи лікаря
...
CHECK (workExperience BETWEEN 1 AND 75);
...
CREATE TABLE advice( -- опис поради
    ...
    dataAdded DATE, -- дата додавання поради
    rating NUMERIC(2,1), -- cередня оцінка користувачів
    doctor_id SMALLINT, -- N лікаря
    category_id SMALLINT -- N категорії
);
...
ALTER TABLE advice
ALTER COLUMN dataAdded SET NOT NULL;
